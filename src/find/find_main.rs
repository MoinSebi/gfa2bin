use crate::core::helper::{merge_u32_to_u64, to_string1, Feature, split_u64_to_u32s};
...
pub fn find_main(matches: &ArgMatches) {
    ...
    if feature == Feature::MWindow{
        ...
        find_wrapper(feature, graph, data, length, output);
    }
...
pub fn find_wrapper(feature: Feature, gfa: NCGfa<()>, file_data: FileData, length: usize, output: &str) {
    ...
    } else if feature == Feature::Block {
        find_block(&gfa, file_data);
    } else if feature == Feature::PWindow {
        find_pwindow(&graph, file_data, mw)
    }
}
...
pub fn find_alignment(graph: &NCGfa<()>, input: FileData, feature: Feature, length: usize) {
    ...
    ff.data = input.data.iter().map(|x| split_u64_to_u32s(*x).0 as u64).collect();
...
pub fn write_matrix(mw: MatrixWrapper, output: &str, win: usize, start: usize, aa: Vec<(usize, Vec<[bool; 2]>)>) {
    let file = File::create(output).unwrap();
    let mut writer = std::io::BufWriter::new(file);
    ...
    for x in aa.iter(){
        ...
        write!(writer, "{}", x.1.iter().map(|x| matric_writer(x)).collect::<String>()).expect("Error writing to file");
            ...
            if node2index.contains_key(&x1) {
                for z in node2index.get(&x1).unwrap() {
                    if window <= *z && *z + window < ll + 1 {
                        vecc.push((graph2.genomes[*genome_id].haplotypes[*haplotype_id].paths[*path_id]
...
pub fn write_subpath(aa: &Vec<(Vec<&[u32]>, usize, usize, usize, usize, usize)>, ll: usize, output: &str, pp: &Pansn<NCPath>) {
    ...
    for y in aa.iter(){
        ...
        if c == ll{
            write!(writer, "{}", pp.genomes[ab.1].haplotypes[ab.2].paths[ab.3].name).expect("Error writing to file");
            write!(writer, "{}", ).expect("Error writing to file");
        }
    }
